...
async function getTechShopStatus() {
  try {
    ...
    console.log(`🖼️  Products with Images: ${productsWithImages}/${totalProducts} (${Math.round(productsWithImages/totalProducts*100)}%)`);
    ...
    if (imageQualityCheck.length > 0) {
      ...
      console.log('\n🖼️  Image Quality:');
      console.log(`  Optimized Images: ${imgStats.optimizedImages}/${imgStats.totalImages} (${Math.round(imgStats.optimizedImages/imgStats.totalImages*100)}%)`);
    }
    ...
    console.log('\n🎯 Next Steps:');
    console.log('  1. Visit http://localhost:5174/products to see your products');
