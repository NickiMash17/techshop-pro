...
async function viewProductsByCategory() {
  try {
    ...
    const products = await Product.find({ category: selectedCategory }).sort({ name: 1 });
...
async function updateProduct() {
  try {
    ...
    const productChoice = parseInt(await question('\nEnter product number: ')) - 1;
    ...
    const description = await question(`Description (${product.description.substring(0, 50)}...): `);
    ...
    const categoryChoice = await question(`Category (current: ${product.category}): `);
    const featured = await question(`Featured (current: ${product.featured ? 'y' : 'n'}) (y/n): `);
    ...
    if (categoryChoice) updates.category = categories[parseInt(categoryChoice) - 1];
